...
func TestReviewSingleFileMarkdown(t *testing.T) {
	...
	err := RunWith(corpusDir, promptsDir, contentFile, "", nil, completer, nil, nil, time.Second)
	if err != nil {
		t.Fatal(err)
	}
...
func TestReviewStdin(t *testing.T) {
	...
	writePrompt(t, promptsDir, "ROLE")
	completer := &scriptedCompleter{steps: []ai.Step{{Text: "ok", StopReason: "end_turn"}}}
	err := RunWith("", promptsDir, "", "", strings.NewReader("piped content"), completer, nil, nil, time.Second)
	if err != nil {
		t.Fatal(err)
	}
...
func TestReviewBatch(t *testing.T) {
	...
	err := RunWith(corpusDir, promptsDir, "", batchDir, nil, completer, nil, &trace, time.Second)
	if err != nil {
		t.Fatal(err)
	}
...
func TestReviewBatchEmptyDirErrors(t *testing.T) {
	...
	err := RunWith(corpusDir, promptsDir, "", t.TempDir(), nil, completer, nil, nil, time.Second)
...
func TestReviewMissingRolePrompt(t *testing.T) {
	completer := &scriptedCompleter{steps: []ai.Step{{Text: "x", StopReason: "end_turn"}}}
	err := RunWith("", t.TempDir(), "", "", nil, completer, nil, nil, time.Second)
